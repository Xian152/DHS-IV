...
***Postnatal Care************* 
****************************** 
// child pnc information varies from surveys and is not avaliable for most surveys
   ...
   if inlist(name,"Indonesia2002") {
		...
		replace m72_skill = 1 if inlist(m72,12,13) 
	...
	*c_pnc_any : mother OR child receive PNC in first six weeks by skilled health worker  //to be decided whether to keep? because m52_skill is missing. 
	...
	if inlist(name,"Indonesia2002") {
		...
		replace c_pnc_any = . if (inlist(m71,199,299,399,998,999)| m72_skill == . ) & (inlist(m51,998,999) | m52_skill == . )
		replace c_pnc_any = 0 if m70 ==0 & m50 ==0
	}
	...
	*c_pnc_eff: mother AND child in first 24h by skilled health worker		
    ...
    replace c_pnc_eff = . if inlist(m51,199,299,399,998,999) |  m52_skill == . | inlist(m71,199,299,399,998,999) | m72_skill == .              
 	replace c_pnc_any = 0 if m70 ==0 | m50 ==0
	}
